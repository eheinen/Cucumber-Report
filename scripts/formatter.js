...
CucumberHTML.DOMFormatter = function(rootNode) {
  ...
  this.examples = function(examples) {
    var examplesElement = blockElement(currentElement.children('div'), examples, 'examples');
    var examplesTable = $('.examples_table', $templates).clone();
    examplesTable.appendTo(examplesElement.children('div'));
  ...
  this.result = function(result) {
    ...
    if (isLastStep) {
      if (currentSteps.find('.failed').length == 0) {
        ...
        currentElement.find('div').removeAttr('open');
      } else {
        currentElement.find('div').attr('open', 'open');
      }
    }
  };
  ...
  this.embedding = function(mimeType, data) {
    ...
    {
      currentStep.append('<img src="' + getReportPath() + data + '">');
    }
    else if (mimeType.match(/^video\//)) 
    {
      currentStep.append('<video src="' + getReportPath() + data + '" type="' + mimeType + '" autobuffer controls>Your browser doesn\'t support video.</video>');
    }
  ...
  this.before = function(before) {
    if(before.status != 'passed') {
      ...
      currentStepIndex = 1;
      populateStepError($('div', currentElement), before.error_message);
    }
  };
  ...
  this.after = function(after) {
    if(after.status != 'passed') {
      ...
      currentStepIndex++;
      populateStepError($('div', currentElement), after.error_message);
    }
  };

  function featureElement(statement, itemtype) {
    var e = blockElement(currentFeature.children('div'), statement, itemtype);

    currentSteps = $('.steps', $templates).clone();
    currentSteps.appendTo(e.children('div'));
...
CucumberHTML.templates = '<div>\
  <section class="blockelement" itemscope>\
    <div class="container">\
      ...
      <pre itemprop="description" class="description">The description goes here</pre>\
    </div>\
  </section>\
\
  <ol class="steps"></ol>\
...
\
  <section class="embed">\
    <br><img itemprop="screenshot" class="screenshot" />\
  </section>\
  <div class="tags"></div>\
  <span class="tag"></span>\
